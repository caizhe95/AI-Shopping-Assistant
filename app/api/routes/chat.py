...
async def chat_endpoint(payload: ChatRequest, session: AsyncSession = Depends(get_db_session)): 
 ...
 if payload.debug: 
  ...
  recommendation = {} 
  comparison = {} 
 return ChatResponse(session_id=result.get('session_id') or payload.session_id, reply=result.get('response') or '', trace=trace, steps=steps, recommendation=recommendation, comparison=comparison) 
